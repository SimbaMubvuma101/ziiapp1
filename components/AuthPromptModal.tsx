...
export const AuthPromptModal: React.FC<AuthPromptModalProps> = ({ onClose }) => {
  ...
  const handleLogin = () => {
    // Store current event/tab params for post-login redirect
    ...
    if (eventId) {
      localStorage.setItem('zii_pending_event', eventId);
      if (tab) {
        localStorage.setItem('zii_pending_tab', tab);
      }
    }
  ...
  const handleRegister = () => {
    // Store current event/tab params for post-registration redirect
    const hash = window.location.hash.split('?')[1] || '';
    const params = new URLSearchParams(hash);
    const eventId = params.get('event');
    const tab = params.get('tab');
